...
- **Ollama** — the local LLM server (`http://localhost:11434`) idea-vault talks to. The only AI
  backend. See [ADR-0003](./adr/0003-ollama-local-only-ai.md).
- **SSE (Server-Sent Events)** — the one-way streaming channel used to push AI tokens to the browser
  as they are generated. See [ADR-0004](./adr/0004-sse-token-streaming.md).
- **Context budget** — the bounded amount of text (idea body + selected memory + recent
  conversation) assembled into a prompt, kept within the local model's limits by `ai::budget`.
- **Degradation** — the defined behavior when Ollama is slow or absent: the UI surfaces a clear
  state and never hangs. See [D20](./05-ai-integration.md).
...
- **Bounded concurrency** — the hard cap (a semaphore) on how many Ollama calls run at once during a
  swarm, protecting a single local machine. See [ADR-0006](./adr/0006-bounded-concurrency-swarm.md).
...
- **AppState** — the shared, cloneable application state (config, index handle, Ollama client,
  concurrency limiter) held by axum handlers.
- **Askama** — the compile-time HTML templating library used for server-rendered pages.
- **HTMX** — the client-side library that turns HTML attributes into AJAX/SSE interactions, avoiding
  a JS SPA.
- **Partial** — an HTML fragment (not a full page) returned to HTMX to swap into the DOM.

## Diagram vocabulary

- **Diagram ID (Dn)** — every diagram in the docs has a stable ID `D1`…`D25`, catalogued in
