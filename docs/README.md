...
> **Status: documentation-first.** No application code exists yet. These docs are the contract the
> codebase will be built against. The top-level [CLAUDE.md](../CLAUDE.md) is the north-star spec;
> everything here elaborates it. If a doc and CLAUDE.md ever disagree, update both in one change.

## How to read the diagrams

Every diagram is authored in a `mermaid` fenced code block and **renders inline on GitHub** — no build
step. Each has a stable ID (**D1**–**D25**) catalogued in [08-diagrams](./08-diagrams.md). To render
...
7. [05-ai-integration](./05-ai-integration.md) — Ollama, SSE, degradation, errors (D3, D11, D20, D24).
...
| [05-ai-integration](./05-ai-integration.md) | Ollama boundary, SSE, degradation, errors | D3, D11, D20, D24 |
...
| [09-web-ui](./09-web-ui.md) | Routes, middleware, templates, HTMX/SSE | D16, D17 |
...
- **UI:** axum + Askama + HTMX + SSE, single binary, no JS build ([ADR-0001](./adr/0001-server-rendered-htmx-over-spa.md), [ADR-0004](./adr/0004-sse-token-streaming.md)).
- **Storage:** markdown = truth, SQLite = rebuildable index ([ADR-0002](./adr/0002-markdown-source-of-truth-sqlite-index.md)).
- **AI:** Ollama local only, `:11434` ([ADR-0003](./adr/0003-ollama-local-only-ai.md)).
...
## What's next (out of scope for these docs)

Scaffolding: `Cargo.toml`, the axum server skeleton, the `src/` modules from
[02-module-reference](./02-module-reference.md), Askama templates, and an example `vault/`.
Per the plan, scaffolding can begin against docs 01–04 while the rest are refined. The container
files ([`Dockerfile`](../Dockerfile), [`docker-compose.yml`](../docker-compose.yml),
[`docker-compose.gpu.yml`](../docker-compose.gpu.yml)) already exist as the deployment contract
([12-deployment](./12-deployment.md)) — they build once the crate is scaffolded.
